...
def simulate_error_probability(probability=0.1):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            stats, created = SimulationStats.objects.get_or_create(pk=2)
            SimulationStats.objects.filter(pk=2).update(total_calls=F('total_calls') + 1)
            ...
            if random.random() < probability:
                ...
                SimulationStats.objects.filter(pk=2).update(simulated_errors=F('simulated_errors') + 1)
...
def get_error_stats():
    ...
    try:
        ...
        stats = SimulationStats.objects.get(pk=2)
